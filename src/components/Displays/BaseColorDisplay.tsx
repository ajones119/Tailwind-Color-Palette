        ...
        <div>
            <div className="grid grid-cols-4 max-w-5xl">
                {colorRows.map((row) => {
                    ...
                    const tooltipContent = copiedColor === color ? 'Copied!' : (colorName ?? color)
                    return (
                        <Tooltip.Root
                            key={row.title}
                            open={isOpen}
                            onOpenChange={(open) => {
                                    ...
                                    else setHoveredColor((prev) => (prev === row.title ? null : prev))
                                }}
                        >
                            <Tooltip.Trigger
                                delay={0}
                            ...
                            >
                                <div className="w-full h-32 md:h-24 lg:h-16 flex items-center justify-center line-clamp-1 transition-colors duration-300" style={{ background: color, color: getTextColorOnBackground(color) }} >
                                    <motion.span
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
                                        exit={{ opacity: 0 }}
