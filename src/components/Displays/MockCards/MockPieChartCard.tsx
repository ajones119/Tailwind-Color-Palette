...
export default function MockPieChartCard() {
  ...
  return (
    ...
    >
      <div className="mb-1 text-lg font-bold" style={{ color: neutral[9] }}>Distribution</div>
      <div className="text-sm leading-relaxed mb-0 border-b pb-5" style={{ color: neutral[6], borderColor: neutral[2] }}>Pie chart with your palette</div>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <PieChart key={isInView ? 'in-view' : 'out-of-view'}>
          ...
          >
            {data.map((_, index) => (
              <Cell key={index} fill={segmentColors[index % segmentColors.length]} stroke={neutral[0]} strokeWidth={2} />
            ))}
          </Pie>
          <Tooltip
            contentStyle={{
