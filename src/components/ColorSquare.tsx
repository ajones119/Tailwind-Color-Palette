...
const ColorSquare = memo(({ value, shadeIndex, isRootColor }: ColorSquareProps) => {
    ...
    return (
        <Tooltip.Root open={hoverOpen || copied} onOpenChange={setHoverOpen}>
            <Tooltip.Trigger
                ...
                className="relative block size-20 lg:size-full aspect-square rounded-sm overflow-hidden cursor-pointer border border-neutral-300 shrink-0 transition-transform duration-300 hover:scale-105"
                onClick={handleClick}
                delay={0}
            >
                <span className="sr-only">Copy color</span>
                <span className="block size-full rounded-sm transition-colors duration-100" style={{ backgroundColor: value }} aria-hidden />
                <div
