...
import { getHarmonyRootsFromHsl, getTextColorOnBackground, hexToHsl, hslToHex } from '../../../lib/color'
...
export const RadialRootInputs = () => {
    ...
    const baseSaturation = baseColorRow?.rootColor ? hexToHsl(baseColorRow.rootColor)?.s ?? 100 : 100
    const baseLightness = baseColorRow?.rootColor ? hexToHsl(baseColorRow.rootColor)?.l ?? 50 : 50
    ...
    const [hue, setHue] = useState<Record<RowKey, number>>(() => ({
        Primary: hexToHsl(colorRows.find(r => r.title === 'Primary')?.rootColor ?? '#000000')?.h ?? 0,
        Secondary: hexToHsl(colorRows.find(r => r.title === 'Secondary')?.rootColor ?? '#000000')?.h ?? 0,
        Tertiary: hexToHsl(colorRows.find(r => r.title === 'Tertiary')?.rootColor ?? '#000000')?.h ?? 0,
    }))
    ...
    const { width, height } = useResizeObserver({ ref: pieBackground, box: 'border-box' })
    ...
    useEffect(() => { slRef.current = { s: saturation, l: lightness } }, [saturation, lightness])
    ...
    pushDragUpdateRef.current = () => {
        startTransition(() => {
            if (harmonyMethod !== 'auto') {
                const primaryHex = hslToHex(hueRef.current.Primary, slRef.current.s, slRef.current.l, 100)
                ...
                if (!row) return
                const newHex = hslToHex(hueRef.current[row], slRef.current.s, slRef.current.l, 100)
                if (!newHex) return
    ...
    useEffect(() => {
        ...
        const primary = colorRows.find(r => r.title === 'Primary')?.rootColor ?? '#000000'
        const secondary = colorRows.find(r => r.title === 'Secondary')?.rootColor ?? '#000000'
        const tertiary = colorRows.find(r => r.title === 'Tertiary')?.rootColor ?? '#000000'
    ...
    useEffect(() => {
        ...
        pieBackground.current.style.background =
            `conic-gradient(from 90deg, hsl(0,${saturation}%,${lightness}%) 0deg, hsl(60,${saturation}%,${lightness}%) 60deg, hsl(120,${saturation}%,${lightness}%) 120deg, hsl(180,${saturation}%,${lightness}%) 180deg, hsl(240,${saturation}%,${lightness}%) 240deg, hsl(300,${saturation}%,${lightness}%) 300deg, hsl(360,${saturation}%,${lightness}%) 360deg)`
    ...
    const syncIndicatorPosition = useCallback((indicator: React.RefObject<HTMLDivElement | null>, h: number) => {
    ...
    }, [hue.Primary, hue.Secondary, hue.Tertiary, syncIndicatorPosition, width, height])

    const updateHueFromPointer = useCallback((clientX: number, clientY: number, row: RowKey) => {
    ...
    const handlePointerDown = useCallback((e: React.PointerEvent, row: RowKey) => {
        e.preventDefault()
    ...
    const handlePointerMove = useCallback((e: React.PointerEvent, row: RowKey) => {
        if (!isDragging.current) return
        updateHueFromPointer(e.clientX, e.clientY, row)
        ...
        if (lastUpdatePos.current) {
            ...
            if (dx * dx + dy * dy >= DRAG_UPDATE_THRESHOLD_PX * DRAG_UPDATE_THRESHOLD_PX) {
                lastUpdatePos.current = { x: e.clientX, y: e.clientY }
                scheduleDragPush()
            }
        }
    ...
    const handlePointerUp = useCallback((e: React.PointerEvent, row: RowKey) => {
        ...
        if (harmonyMethod !== 'auto') {
            const primaryHex = hslToHex(hueRef.current.Primary, saturation, lightness, 100)
            if (primaryHex) {
                updateRowRootColor('Primary', primaryHex)
                alignWithHarmony('Primary')
            }
        } else {
            const newHex = hslToHex(hueRef.current[row], saturation, lightness, 100)
            if (newHex) {
                updateRowRootColor(row, newHex)
                alignNeutralWithPrimary()
            }
    ...
    }, [harmonyMethod, saturation, lightness, updateRowRootColor, alignWithHarmony, alignNeutralWithPrimary])
        ...
        <div className="grid md:grid-cols-2 lg:flex lg:flex-col gap-4">
                ...
                >
                    ...
                    <div
                        ...
                        style={{
                            ...
                            borderColor: getTextColorOnBackground(baseColorRow?.rootColor ?? '#000000'),
                            color: getTextColorOnBackground(baseColorRow?.rootColor ?? '#000000'),
                        }}
                        role="slider"
            ...
            <div className="flex flex-col gap-1.5 hidden">
                <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs tabular-nums text-neutral-500">{Math.round(hue.Primary)}°</span>
                </div>
                <Slider.Root
                    value={hue.Primary}
                    min={0}
                ...
                >
                    <Slider.Control className="flex items-center w-full h-5 cursor-pointer">
                        <Slider.Track className="relative flex-1 h-3 lg:h-2 rounded-full" style={{ background: hueGradient }}>
                            ...
                            <Slider.Thumb
                                ...
                                style={{ background: `hsl(${hue.Primary}, ${saturation}%, ${lightness}%)` }}
                            />
                        </Slider.Track>
                    </Slider.Control>
                </Slider.Root>
            ...
            <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                    <label htmlFor="saturation-slider" className="text-base lg:text-xs font-mono text-neutral-600">Saturation</label>
                    <span className="text-base lg:text-xs font-mono tabular-nums text-neutral-500">{Math.round(saturation)}%</span>
                </div>
                <Slider.Root
                ...
                >
                    <Slider.Control className="flex items-center w-full h-5 cursor-pointer">
                        <Slider.Track className="relative flex-1 h-3 lg:h-2 rounded-full" style={{ background: lightShade }}>
                            <Slider.Indicator className="h-full rounded-full" style={{ background: baseShade }} />
                            <Slider.Thumb
            ...
            <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                    <label htmlFor="lightness-slider" className="text-base lg:text-xs font-mono text-neutral-600">Lightness</label>
                    <span className="text-base lg:text-xs font-mono tabular-nums text-neutral-500">{Math.round(lightness)}%</span>
                </div>
                <Slider.Root
                    id="lightness-slider"
                    value={lightness}
