...
const HARMONY_METHODS: HarmonyMethod[] = [
    'analogous', 'complementary', 'triadic', 'square',
    'rectangle', 'split-complementary', 'split-triadic', 'tetradic',
]
...
export default function RootColors() {
    ...
    const { updateRowRootColor, alignWithHarmony, setHarmonyMethod } = useColors()
    ...
    const randomize = () => {
        ...
        const harmony = HARMONY_METHODS[Math.floor(Math.random() * HARMONY_METHODS.length)]
    ...
    return (
        <div className="flex flex-col gap-8">
            <div>
                <h2 className="text-2xl lg:text-lg font-bold font-title mb-1">Root Colors</h2>
                <div className="flex items-center gap-2 flex-wrap">
                    <ToggleGroup<Mode> className="flex gap-1" value={[mode]} onValueChange={handleModeChange} defaultValue={[DEFAULT_MODE]}>
                    <Toggle aria-label="Manual Inputs" value={"inputs" as Mode} className={`active:translate-y-0.5 cursor-pointer size-10 lg:size-6 flex items-center justify-center rounded-sm bg-neutral-100 ${mode === 'inputs' ? 'bg-neutral-200' : ''}`}>
                        <DialIcon className="size-6 lg:size-4" />
                    </Toggle>
                    <Toggle aria-label="Color Wheel" value={"radial" as Mode} className={`active:translate-y-0.5 cursor-pointer size-10 lg:size-6 flex items-center justify-center rounded-sm bg-neutral-100 ${mode === 'radial' ? 'bg-neutral-200' : ''}`}>
            ...
            >
                <motion.div
                    ...
                    exit={mode === 'inputs' ? { opacity: 0, scale: 1, y: 0} : { opacity: 0, scale: 0.95, y: 0}}
