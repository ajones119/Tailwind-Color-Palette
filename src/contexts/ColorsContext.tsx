...
const VALID_HARMONY: HarmonyMethod[] = [
  "auto",
  "analogous",
  "complementary",
  "triadic",
...
function parseHarmonyParam(value: string | null): HarmonyMethod | null {
  ...
  return (
    VALID_HARMONY.includes(v as HarmonyMethod) ? v : null
  ) as HarmonyMethod | null;
}

function colorRowsToParams(
  rows: ColorRow[],
...
function getInitialStateFromUrl(): {
  ...
  if (typeof window === "undefined")
    return { rows: DEFAULT_ROWS, harmony: defaultHarmony };
  ...
  const rows: ColorRow[] = DEFAULT_ROWS.map((row) => {
    const hex =
      row.title === "Primary"
        ? (primary ?? row.rootColor)
        : row.title === "Secondary"
          ? (secondary ?? row.rootColor)
          : row.title === "Tertiary"
            ? (tertiary ?? row.rootColor)
            : row.title === "Neutral"
              ? (neutral ?? row.rootColor)
              : row.rootColor;
    return {
      ...row,
      rootColor: hex,
      colors: generateShadesFromBaseColor(hex, SHADE_KEYS.length),
    };
  });
...
export const ColorsProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [harmonyMethod, setHarmonyMethod] = useState<HarmonyMethod>(
    initial.harmony,
  );
  ...
  const updateRowRootColor = (title: AnchorTitle, rootColor: string) => {
    ...
    setColorRows((prev) =>
      prev.map((r) =>
        r.title === title ? { ...r, rootColor, colors: shades } : r,
      ),
    );
  };
    ...
    setColorRows((prev) => {
      ...
      return prev.map((r) =>
        r.title === "Neutral"
          ? {
              ...r,
              rootColor: neutralHex,
              colors: generateShadesFromBaseColor(
                neutralHex,
                SHADE_KEYS.length,
              ),
            }
    ...
    setColorRows((prev) => {
      ...
      return prev.map((r) => {
        if (r.title === "Primary")
          return {
            ...r,
            rootColor: primaryHex,
            colors: generateShadesFromBaseColor(primaryHex, SHADE_KEYS.length),
          };
        if (r.title === "Secondary")
          return {
            ...r,
            rootColor: secondaryHex,
            colors: generateShadesFromBaseColor(
              secondaryHex,
              SHADE_KEYS.length,
            ),
        ...
        if (r.title === "Tertiary")
          return {
            ...r,
            rootColor: tertiaryHex,
            colors: generateShadesFromBaseColor(tertiaryHex, SHADE_KEYS.length),
          };
        if (r.title === "Neutral")
          return {
            ...r,
            rootColor: neutralHex,
            colors: generateShadesFromBaseColor(neutralHex, SHADE_KEYS.length),
          };
        return r;
      });
    });
  ...
  const alignWithHarmony = (anchor: AnchorTitle) => {
    setColorRows((prev) => {
      ...
      if (!primaryHex || !secondaryHex || !tertiaryHex || !neutralHex)
        return prev;
