...
export function parseColorToHex(input: string): string | null {
  ...
  try {
    ...
    return alpha < 1
      ? color.toString({ format: 'hex', collapse: false }) + Math.round(alpha * 255).toString(16).padStart(2, '0')
 ...
 */
export function getTextColorOnBackground(background: string): '#000000' | '#ffffff' {
  try {
...
export function setAlphaOnColor(hex: string, alphaPercent: number): string {
  try {
    ...
    return c.alpha < 1
      ? c.toString({ format: 'hex', collapse: false }) + Math.round(c.alpha * 255).toString(16).padStart(2, '0')
 ...
 */
export function formatColorForTheme(hex: string, format: ThemeExportFormat): string {
  try {
    const c = new Color(hex)
    if (format === 'hex') {
      const alpha = c.alpha ?? 1
      return alpha < 1
        ? c.toString({ format: 'hex', collapse: false }) + Math.round(alpha * 255).toString(16).padStart(2, '0')
        : c.toString({ format: 'hex' })
    }
    if (format === 'hsl') {
...
export function hslToHex(h: number, s: number, l: number, a: number = 100): string | null {
  try {
...
export function rgbToHex(r: number, g: number, b: number, a: number = 100): string | null {
  try {
    const alpha = Math.max(0, Math.min(1, a / 100))
 ...
 */
export function generateShadesFromBaseColor(baseColor: string, numberOfShades: number = 11): string[] {
  ...
  const toHex = (c: Color) =>
    baseAlpha < 1
      ? c.toString({ format: 'hex', collapse: false }) + Math.round(baseAlpha * 255).toString(16).padStart(2, '0')
      : c.toString({ format: 'hex' })
...
export type HarmonyMethod = 'auto' | 'analogous' | 'complementary' | 'triadic' | 'square' | 'rectangle' | 'split-complementary' | 'split-triadic' | 'tetradic';

export function getHarmonyRootsFromHsl(hsl: Hsl, method: HarmonyMethod): { secondary: Hsl; tertiary: Hsl } {
  const { h, s, l } = hsl;
